...
class CustomDebitTransactionDialog(context: Context, resid: Int, user: UserResult) : Dialog(context, resid), View.OnClickListener{
    var user = user
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.debit_dialog_transaction)
        ...
        description=findViewById(R.id.edt_enter_description)
        setdata()
    }
    ...
    private var eventlistener=object : ValueEventListener{
        override fun onDataChange(dataSnapshot: DataSnapshot) {
            if(dataSnapshot.exists()){
                if(dataSnapshot.child("debit").value.toString().equals("0")){
    ...
    private fun addnewdata() {
        subref.child("debit").setValue(amount.text.toString())
        subref.child("credit").setValue("0")
        setbalance()
        dismiss()
    }
    ...
    private fun editdata(dataSnapshot: DataSnapshot) {
        subref.child("debit").setValue(amount.text.toString())
        subref.child("credit").setValue(dataSnapshot.child("credit").value.toString())
        setbalance()
        dismiss()
    }
}